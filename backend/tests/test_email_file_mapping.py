...
from app.services.email_mapping.email_file_mapping import map_by_email, sync_email_stage, email_input_signature
from app.api import file_workflow_state as storage
...
class EmailHandoffTests(unittest.TestCase):
    ...
    def test_email_dump_query_parameters(self):
        from app.repositories.email_dump_service import fetch_email_dump, QUERY
        engine = MagicMock()
        execute = engine.connect.return_value.__enter__.return_value.execute
        execute.return_value.fetchall.return_value = []
        with patch.dict(sys.modules, {"app.config.database": MagicMock(engine=engine)}):
