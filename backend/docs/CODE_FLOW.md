...
flowchart TD
    ...
    HTTP --> API[backend/app/routes FastAPI routes]
    API --> Services[backend/app/services existing Python logic]
    Services --> Repositories[backend/app/repositories]
    Repositories --> DB[backend/app/models and database]
    DB --> MySQL[MySQL]
    API --> State[HTTP workspace snapshots and manifests]
    State --> Storage[Existing named-school storage]
```

`backend/app/main.py` retains `app` and `create_app()`, registers health/student browsing
...
4. **Run mapping** posts to the API, which invokes the unchanged
   `map_students()` and `build_exports()` in `backend/app/services/admission_mapping/`.
...
`backend/app/routes/student_mapping.py` retains `/mapping/health` and `/mapping/students`.
...
dropped by removing the source code. `backend/app/scripts/init_db.py` registers only `Student`
and can create the missing `students` table when explicitly run.

## Shared Python helpers and tools

`backend/app/utils/workbook_operations.py`, `table_queries.py` and
`school_statistics.py` own workbook operations, table queries and statistics.
`backend/app/api/file_workflow_helpers.py` preserves their original import paths.
...
`backend/app/scripts/admission_mapping_cli.py` runs CSV/XLSX admission mapping without a browser.
Its direct/module entry points remain. The tested public admission wrapper under
`backend/app/scripts/admission_mapping/admission_file_mapping.py` delegates to the same CLI.
`backend/app/services/admission_mapping/admission_file_mapping.py` remains the public
service facade used by API/tests. `backend/app/scripts/init_db.py` registers the retained student ORM model
