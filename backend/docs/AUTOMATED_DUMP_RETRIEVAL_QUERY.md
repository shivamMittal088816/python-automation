...
Source: `backend/app/repositories/admission_dump_service.py`.
...
- [Query definitions and normalization](../app/repositories/admission_dump_service.py)
- [Fetch endpoint and snapshot replacement](../app/routes/file_workflow_routes/file_inputs.py)
- [Distinct student count and overview](../app/utils/school_statistics.py)
- [Session and data flow](SESSION_AND_DATA_FLOW.md)
...
- [Repository](../app/repositories/email_dump_service.py): `QUERY` and `fetch_email_dump(values)`.
- [Route](../app/routes/file_workflow_routes/email_mapping.py): `email_map()`.
- Default endpoint: `POST /api/v1/mapping/email-mapping/run`.
- [Classification](../app/services/email_mapping/email_row_classification.py): Python matching after retrieval.
...
- [Repository](../app/repositories/user_student_repository.py): `UserStudentRepository.get_students(cursor=None, limit=25)`.
- [Route](../app/routes/student_mapping.py): `get_students()`.
...
[Implementation](../app/main.py): `check_database_connection()`, called
...
[Engine configuration](../app/config/database.py) enables
...
The [Student model](../app/models/student_model.py) registers `students`:
