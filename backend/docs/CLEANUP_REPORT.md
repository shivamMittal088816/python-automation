...
The runtime architecture is `frontend/` React/Vite/Tailwind/Router → HTTP → `backend/app/`
FastAPI → services → database repositories/models → MySQL. Supporting directories
are `backend/app/scripts/`, `tests/`, `storage/` and `logs/`. Framework identification came from
`backend/app/main.py`, routes and dependency metadata, not README terminology.
...
|---|---|
| Entire `web/` (36 Python files) | Nine pages/components replaced by fixture-verified React/HTTP workflows; no production backend/app/script imports; reusable operations already active in backend |
...
| `backend/app/scripts/admission_mapping/admission_mapping_cli.py` | Unreferenced import-only alias; canonical CLI and tested public wrapper remain |
...
| `backend/app/services/base_matcher.py` | Abstract interface for removed matcher pipeline, no subclasses/callers |
| `backend/app/services/reviewer_processor.py` | Comment-only placeholder; live Review students updates use existing service/repositories |

Complete deleted-source inventory:

- `app.py`
- `main.py`
- `backend/app/scripts/admission_mapping/admission_mapping_cli.py`
- `backend/app/schemas/student_schema.py`
- `backend/app/schemas/upload_request_schema.py`
- `backend/app/schemas/upload_schema.py`
- `backend/app/schemas/validation_schema.py`
- `backend/app/services/base_matcher.py`
- `backend/app/services/reviewer_processor.py`
- `web/__init__.py`
- `web/app.py`
...
|---|---|
| `backend/app/scripts/admission_mapping/admission_file_mapping.py` | Public imports and real script/module CLI styles exercised by `test_admission_modules.py` |
| `backend/app/scripts/admission_mapping/__init__.py` | Package needed for the supported module entry point |
| `backend/app/services/admission_mapping/admission_file_mapping.py` | Public facade imported by active API and business tests |
...
| `backend/app/api/file_workflow_helpers.py` | Active search, Preview screen transfers, workbook conversion and overviews; function bodies unchanged |
| `backend/app/api/file_workflow_storage.py` | Active school save/restore, staging and rollback; functions unchanged |
| Other file-workflow modules/schema | React depends on their existing HTTP boundary |
| `backend/app/scripts/init_db.py` and ORM models | Legitimate database administration and model registration |
| `backend/app/repositories/user_package_repository.py` | Independent SQL package/year utility; retained conservatively although no active caller was found |
...
parser installation, then restarted with the original `app.main:app --reload`
...
`app.api.file_workflow_helpers`. Email and full-name/class storage tests now
import `app.api.file_workflow_storage`. Matching/classification, duplicates,
...
Removed cache-only `apps/`, `src/`, `backend/app/dtos/`, `backend/app/handlers/`,
`backend/app/models/`, `backend/app/repositories/`, and source-directory Python caches.
