import { ClearMappingFileButton } from '../../components/common/ClearMappingFileButton';
...
export function AdmissionMappingForm() {
  ...
  useEffect(() => { setDraft({}); setPreview(null); setPreviewBusy(false); setPreviewError(''); }, [id, workspace.files.school?.version, workspace.files.dump?.version]);
  useEffect(() => {
    ...
    admissionMappingApi.configuration(selection(), controller.signal).then(data => {
  ...
  <section className="min-w-0 rounded-xl border border-blue-200 bg-blue-50/50 p-4">
    ...
    <Button primary disabled={!!busy || previewBusy || !!previewError || config.missing_columns.length > 0 || !workspace.files.dump?.school_index} onClick={() => run('Mapping students...', revision => admissionMappingApi.map(selection(), revision))}>Run mapping</Button>
  </section>
...
function UploadedDumpSchoolIndex() {
  ...
  return <form className="mt-3 space-y-3" onSubmit={event => {
    ...
    run('Saving school index...', revision => admissionMappingApi.schoolDetails({ school_index: index.trim() }, revision));
  }}>
