...
import { PassResultsBadge } from '../../components/common/PassResultsBadge';
...
export function AdmissionMappingForm() {
  ...
  useEffect(() => { setDraft({}); setPreview(null); }, [id, workspace.files.school?.version, workspace.files.dump?.version]);
  useEffect(() => {
  ...
  const [source, setSource] = useState(sqlDump ? 'Fetch from SQL' : values.admission_dump_source || 'Upload dump file');
  const [selectedFullName, setSelectedFullName] = useState(values.admission_round_two_name_column ?? null);
  const schoolColumns = config?.school_columns || [];
  const fullName = selectedFullName === '' || schoolColumns.includes(selectedFullName) ? selectedFullName : schoolColumns.find(column => ['fullname', 'studentfullname', 'studentname'].includes(String(column).toLowerCase().replace(/[^a-z0-9]/g, ''))) || '';
  const passOneComplete = Object.keys(workspace.exports.admission).length > 0;
  const reviewCount = workspace.exports.admission['review.xlsx'] || 0;
  ...
  <section className="min-w-0 rounded-xl border border-blue-200 bg-blue-50/50 p-4">
    <h3 className="text-sm font-semibold text-blue-900">Pass 1: Admission number + first name <RunColumns stage="admission" pass="1" /></h3>
    ...
    <Button primary disabled={!!busy || previewBusy || !!previewError || config.missing_columns.length > 0 || !workspace.files.dump?.school_index} onClick={() => run('Mapping students...', () => admissionMappingApi.map(id, selection()), { invalidateEmailSource: true })}>Run pass 1</Button>
  </section>
  <section className="min-w-0 rounded-xl border border-indigo-200 bg-indigo-50/50 p-4">
    <h3 className="text-sm font-semibold text-indigo-900">Pass 2: Admission number + sorted full name <RunColumns stage="admission" pass="2" /></h3>
    <p className="my-3 text-sm leading-5 text-slate-600">Retry only first-name mismatches in Review. Match the admission number, then compare full names in lowercase with spaces removed and characters sorted. Equal names move to Matched; others stay in Review.</p>
    <div className="mb-3"><Select label="School full name column" options={[{ value: '', label: 'Choose a full name column' }, ...schoolColumns]} value={fullName} disabled={!!busy} onChange={event => setSelectedFullName(event.target.value)} /></div>
    <Button disabled={!!busy || previewBusy || !!previewError || !passOneComplete || !reviewCount || !fullName || config.missing_columns.length > 0 || !workspace.files.dump?.school_index} onClick={() => run('Running admission pass 2...', () => admissionMappingApi.secondPass(id, fullName), { invalidateEmailSource: true })}>Run pass 2</Button>
    <p className="mt-2 text-xs text-slate-500">{!passOneComplete ? 'Run pass 1 first.' : reviewCount ? `${reviewCount.toLocaleString()} records in Review. Only first-name mismatches are retried; duplicate checks still apply.` : 'No records in Review to recheck.'}</p>
  </section>
</div></>}{Object.keys(workspace.exports.admission).length > 0 && <><SectionHeader step="03" title="Mapping results" description="Saved results from your last run. Select Run pass 1 to run again." /><PassResultsBadge stage="admission" /><Metrics items={[["Rows in identical duplicate groups", config?.duplicate_rows], ["Duplicate rows dropped", config?.dropped_rows]]} /><p className="caption">One copy of each completely identical school row is kept before admission mapping.</p><div className="grid gap-3 sm:grid-cols-3">{[['matched.xlsx', 'Matched', 'Admission number matches uniquely, with matching first names in pass 1 or sorted full names in pass 2; returns the dump username and user ID.'], ['review.xlsx', 'Review', 'Admission number is blank, duplicated in the school file, or occurs more than once in the dump; or the name comparison failed or a name is missing. Dump row numbers include the header as row 1.'], ['not_matched.xlsx', 'Not matched', 'Admission number not found in the dump.']].map(([name, label, description]) => <Card key={name}><Badge>{label}</Badge><p className="my-2 text-2xl font-semibold">{workspace.exports.admission[name]}</p><p className="mb-3 caption">{description}</p><ActionLink to={`/admission_preview_page?file=${name}`}>Preview {label.toLowerCase()}</ActionLink></Card>)}</div></>}</div>;
}
