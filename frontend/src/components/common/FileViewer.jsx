...
function LoadedViewer({ kind, title, id, workspace, run, busy, saved }) {
  ...
  const request = useRequest(signal => fileApi.table(id, kind, params, signal), [id, kind, JSON.stringify(params)]);
